...
def CompareNames(data, cites):
    ...
    for cat, lname, fname in zip(data['I request'],data['Last name'], data['First name']):
...
def CompareNamesN(data, cites):
    rpublist = {}
    apublist = {}
    membershipColumn = 'I request'
    LnameTerm = 'Last name'
    FnameTerm = 'First name'
    searchterm1 = 'regular'
    searchterm2 = 'associated'
    # counter = 0
    for cat, lname, fname in zip(data[membershipColumn],data[LnameTerm], data[FnameTerm]):
        if isinstance(cat, str):
            if 'regular' not in cat and 'associated' not in cat:
                continue
        if isinstance(lname, str):
                ...
                if len(paper) > 0:
                    ...
                    for author in hd_list[paper]['authors']:
                        if author.split(', ')[0].rstrip().lower() == lname.rstrip().lower():
                            if author.split(', ')[1].rstrip().split(' ')[0].lower() == fname.rstrip().lower():
                                match = True
                                temp = cites[cites.PMID == int(paper)][["Authors", "Title", "Citation"]].values
                                if searchterm1 in str(cat):
                                    if paper in rpublist.keys():
                                        rpublist[paper].name.append(lname.rstrip().lower() + ' ' + fname.rstrip().lower())
                                    else:
                                        rpublist[paper] = PMatch(lname.rstrip().lower() + ' ' + fname.rstrip().lower(),paper, temp[0][0], temp[0][1], temp[0][2] )
                                    # rpublist.add(lname.rstrip().lower() + '\t' + fname.rstrip().lower() + '\t' + temp[0][0] + '\t' + temp[0][1] + ' ' + temp[0][2] + ' PMID:' + paper)
                                if searchterm2 in str(cat):
                                    if paper in apublist.keys():
                                        apublist[paper].name.append(
                                            lname.rstrip().lower() + ' ' + fname.rstrip().lower())
                                    else:
                    ...
                    if match == False:
                        for author in hd_list[paper]['authors']:
                            ...
                            if normalize(author.split(', ')[0]).rstrip().lower() == normalize(lname.rstrip().lower()):
                                if normalize(author.split(', ')[1]).rstrip().split(' ')[0].lower() == normalize(fname.rstrip().lower()):
                                    print(author + ' : ' + fname + ' ' + lname)
                                    # match = True
                                    temp = cites[cites.PMID == int(paper)][["Authors", "Title", "Citation"]].values
                                    if searchterm1 in str(cat):
                                ...
                                elif len(author.split(', ')[1].rstrip().split(' ')[0].lower()) == 1 and normalize(author.split(', ')[1]).rstrip().split(' ')[0].lower() == normalize(fname.rstrip()[0].lower()):
                                    print(author + ' : ' + fname + ' ' + lname)
                                    # match = True
                                    temp = cites[cites.PMID == int(paper)][["Authors", "Title", "Citation"]].values
                                    if searchterm1 in str(cat):
...
if __name__ == '__main__':
    ...
    for item in rpublist.values():
        ...
        for i in item.authors.split(', '):
            if ' ' in i:
                ...
                elif any(normalize(namematch).lower() in normalize(sub) for sub in item.name):
                    runner = p.add_run(i)
                    runner.bold = True
    ...
    print("doing underline file")   # add document for underlined partial matches
    counter = 0
    document_und = Document()
    document_und.add_heading('Copy of main document with only partial matched names highlighted for checking purposes')
    document_und.add_heading('Studies Conducted in Collaboration or in Affiliation with Regular EHDN Members in ' + quarter , level=1)
    for item in rpublist.values():
        counter += 1
        p = document_und.add_paragraph(str(counter) + '.\t')
    ...
    counter = 0
    document_und.add_heading('Studies Conducted in Collaboration or in Affiliation with Associate EHDN Members in ' + quarter, level=1)
    for item in apublist.values():
        counter += 1
        p = document_und.add_paragraph(str(counter) + '.\t')
    ...
    while True:
        try:
            ...
            pathout = fd.askdirectory(title='Please select the folder to save to')
            document_und.save(pathout + '/huntington_publications_' + quarter + '_' + datetime.today().strftime('%Y-%m-%d') + '_underlined_partials_for_checking' + '.docx')
            print('document_underlined saved')
            break
        except:
